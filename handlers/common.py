...
async def cmd_start(message: Message, state: FSMContext, command: CommandObject) -> None:
    ...
    await message.answer(
        ...
        reply_markup=main_menu(admin, settings.payments_enabled, settings.esim_enabled, settings.temp_email_enabled),
...
async def nav_main(call: CallbackQuery, state: FSMContext) -> None:
    ...
    await safe_edit(
        ...
        main_menu(is_admin(call.from_user.id), settings.payments_enabled, settings.esim_enabled, settings.temp_email_enabled),
    )
    await call.answer()
...
async def cmd_menu(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        await main_menu_text(message.from_user.id),
        reply_markup=main_menu(is_admin(message.from_user.id), settings.payments_enabled, settings.esim_enabled, settings.temp_email_enabled),
        disable_web_page_preview=True,
    )
