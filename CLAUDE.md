...
- **No migration framework.** Schema changes (new columns) need a manual `ALTER TABLE` on the live `smsbot.db`, or delete the DB to recreate via `db.init_db()` (SQLAlchemy `create_all` does not alter existing tables). Several columns (`held`, `chat_id`, `message_id`, `hero_released`) were added this way.
